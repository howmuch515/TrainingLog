...
Vue.component('record-modal', {
    ...
    methods: {
        ...
        switch_editable: function(record_id) {
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="v in selected_record_list" v-if="editable_flag && selected_record_id === v.record_id" :class="{ record_head_of_date: isHead(v.date) }">
                                    <td><input v-model="submit_date" type="date" class="col form-control"></td>
                                    <td><select v-model="submit_menu_id" class="col form-control">
                                        <option v-for="v in selected_menu_list" :value="v.menu_id">
                                            {{ v.menu_name }}(STEP: {{ v.menu_step }})
                                        </option>
                                    </select></td>
                                    <td><input v-model="submit_count" type="number" class="col form-control"></td>
                                    <td><button @click="submit_log" class="col btn btn-primary">Submit</button></td>
                                ...
                                <tr v-else>
                                    ...
                                    <td>{{ v.count }}</td>
                                    <td><button @click="switch_editable(v.record_id)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 16" width="14" height="16"><path fill-rule="evenodd" d="M0 12v3h3l8-8-3-3-8 8zm3 2H1v-2h1v1h1v1zm10.3-9.3L12 6 9 3l1.3-1.3a.996.996 0 011.41 0l1.59 1.59c.39.39.39 1.02 0 1.41z"></path></svg></button></td>
